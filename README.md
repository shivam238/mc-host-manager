...
- `build.sh` / `build.bat` - build one-file executable (short command)
- `release.sh` / `release.bat` - one-click install deps + build + package
- `build_executable.sh` / `build_executable.bat` - compatibility wrappers
...
- You do not need to run `build_executable.*` for normal testing.
...
bash build.sh
```
Output: `dist/mc-host-manager`

### Windows
```bat
build.bat
```
Output: `dist\mc-host-manager.exe`
...
- Fast one-click:
  ...
  - Windows: `release.bat`
- Advanced/manual:
  - Linux/macOS: `bash package_release.sh`
  - Windows: `package_release.bat`
